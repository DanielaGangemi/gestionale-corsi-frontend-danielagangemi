...
export function Course() {
    ...
    for (let i = 0; i < courses.length; i++) {
        if (courses[i].ID_C == id) {
            course = courses[i];
            break; // Esci dal ciclo una volta trovato il corso
        }
    }
    return (
        <>
            <div className={courseStyle.margin}>
                ...
                <p>Durata: {course.Durata} mesi</p>
                <h5>{course.Descrizione_breve}</h5>
                <p>{course.Descrizione_completa}</p>
            </div>
        </>
    );
}